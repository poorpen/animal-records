...
from src.domain.animal_type.value_objects import AnimalTypeName, AnimalTypeID
...
class GetAnimalType(AnimalTypeUseCase):
    ...
    async def __call__(self, animal_type_int: int) -> AnimalTypeDTO:
        return await self._uow.animal_type_reader.get_type_by_id(animal_type_int)


class CreateAnimaType(AnimalTypeUseCase):
    async def __call__(self, animal_type_dto: CreateAnimalTypeDTO) -> AnimalTypeDTO:
        animal_type = AnimalType.create(animal_type=AnimalTypeName(animal_type_dto.type))
        try:
            ...
            raise
        animal_type.update(type_id=AnimalTypeID(animal_id))
...
class ChangeAnimalType(AnimalTypeUseCase):
    async def __call__(self, animal_type_dto: ChangeAnimalTypeDTO) -> AnimalTypeDTO:
        animal_type = await self._uow.animal_type_repo.get_type_by_id(AnimalTypeID(animal_type_dto.id))
        animal_type.update(animal_type=AnimalTypeName(animal_type_dto.type))
        try:
...
class DeleteAnimalType(AnimalTypeUseCase):
    ...
    async def __call__(self, animal_type_id: int) -> None:
        try:
            await self._uow.animal_type_repo.delete_type(AnimalTypeID(animal_type_id))
            await self._uow.commit()
