...
def add_animal_type(animal: Animal, type_id: int) -> None:
    ...
    if type_of_this_animal in animal.animal_types:
        raise AnimalAlreadyHaveThisType(animal.id.to_id(), type_of_this_animal.animal_type_id.to_id())
...
def change_animal_type(animal: Animal, old_type_int: int, new_type_id: int) -> None:
    ...
    if exist_old_type and exist_new_type:
        raise AnimalAlreadyHaveThisTypes(animal_id=animal.id.to_id(), old_type=old_type_int, new_type=new_type_id)
    elif exist_new_type:
        raise AnimalAlreadyHaveThisType(animal_id=animal.id.to_id(), type_id=new_type_id)

    animal_type = animal.get_animal_type(old_type_int)
    index_animal_type = animal.animal_types.index(animal_type)
    animal.animal_types[index_animal_type].update(animal_type_id=AnimalTypeID(new_type_id))
...
def delete_animal_type(animal, animal_type_id: int) -> None:
    ...
    if len(animal.animal_types) == 1 and animal.animal_types[0] == type_of_this_animal:
        raise AnimalOnlyHasThisType(animal.id.to_id, type_of_this_animal.animal_type_id.to_id)
    elif type_of_this_animal not in animal.animal_types:
        raise AnimalNotHaveThisType(animal.id.to_id, type_of_this_animal.animal_type_id.to_id)
